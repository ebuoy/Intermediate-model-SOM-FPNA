...
class PCSOM:
    def __init__(self, data, connexion_matrices, threshold=switch_threshold, eps_s=epsilon_start, eps_e=epsilon_end, sig_s=sigma_start, sig_e=sigma_end, ep_nb=epoch_nbr):
        ...
        self.nodes = np.empty((neuron_nbr, neuron_nbr), dtype=PCNeuron)
        for x in range(neuron_nbr):
            for y in range(neuron_nbr):
                self.nodes[x, y] = PCNeuron(x, y, data_shape, data_min, data_max, connexion_matrices[x][y], available_neighbours(x, y))
                ...
                for i in range(5):
                    for j in range(5):
                        if self.nodes[x, y].connection_matrix[i, j] != 0:
                            input_vertex = PCSOM.get_index(i, 'i')+str(x)+','+str(y)
                            output_vertex = PCSOM.get_index(j, 'o')+str(x)+','+str(y)
                            e = Edge(input_vertex, output_vertex, PCSOM.neurons_only_weight(i, j))
