...
class SOM:
    ...
    def train(self,k,nbiter,f=gauss,dist_vect=distquad,dist_win_neur=distquad):
        ...
        self._nodes[iwin,jwin]._weight+=eps*gauss(dist_vect(self._nodes[iwin,jwin]._weight,vector),sig)*(vector-self._nodes[iwin,jwin]._weight)
        ...
        for i in range(self._n):
            for j in range(self._n):
                if i!=iwin or j!=jwin:
                    coeff_dist_win_ij=gauss(dist_win_neur(self._nodes[iwin,jwin],self._nodes[i,j],sig))
